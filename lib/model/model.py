...
def spectra_cnn_transformer(num_output=2, PEV, drop_out_rate=0, weights):
...
class learnable_PEV_ct_weights:
    def __init__(self, device):
        self.path =  os.getcwd() + '\\lib\\model\\learnable_PEV.pth'
        print(self.path)
...
def spectra_cnn(num_output=2, PEV, drop_out_rate=0,weights):
...
class poly_concate_c_weights:
    ...
    def __init__(self, device):
        self.path =  os.getcwd() + '\\lib\\model\\poly_concate.pth'
        print(self.path)
        self.checkpoint = torch.load(self.path, map_location=device)
